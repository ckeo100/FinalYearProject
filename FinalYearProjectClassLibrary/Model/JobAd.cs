...
namespace FinalYearProjectClassLibrary.Model
{
    ...
    public class JobAdModel
    {
        ...
        public List<JobAd> GetLocalJobAd(double currentLatitude, double currentLongitude)
        {
            ...
            double searchDistanceInKM = 10;
            //radius of earth
            double radiusOfTheEarthInKm = 6371;
            double maxLat = currentLatitude + mathService.ConvertDegreesToRadians(searchDistanceInKM/radiusOfTheEarthInKm);
            double minLat = currentLatitude - mathService.ConvertDegreesToRadians(searchDistanceInKM/radiusOfTheEarthInKm);
            double maxLong = currentLongitude + mathService.ConvertDegreesToRadians(Math.Asin(searchDistanceInKM / radiusOfTheEarthInKm)) / Math.Cos(mathService.ConvertDegreesToRadians(currentLatitude));
            double minLong = currentLongitude - mathService.ConvertDegreesToRadians(Math.Asin(searchDistanceInKM / radiusOfTheEarthInKm)) / Math.Cos(mathService.ConvertDegreesToRadians(currentLatitude));
            List<JobAd> JobAds = new List<JobAd>();
